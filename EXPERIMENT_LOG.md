...
## Q2: availability-conditioned robustness — cross-domain, decode compounding, cliff

**Status:** measured; accepted by the researcher — no non-free regime, no calibration
**Track:** measured-quality stress, closed loop back to AX4 / Q2 calibration
**Date:** 2026-08-02
**Config:** `configs/experiment/q2-stress.toml` (`[q2_*_probe]`, `[q2_*_gate]`)
**Protocol:** `docs/Q2_PROTOCOL.md` (frozen). CLI impl + runs authorized after Q1/Q1-B accepted.
**Checkpoint/data:** frozen OLMoE-1B-7B-0125 base (revision 9b0c1aa…), null-drop only (renormalize dropped), AX4 anchor incidence 0.009, one expert/layer unless swept. Cross-domain uses WikiText-2 (in-family ref) + gsm8k math (word problems) from local parquet only — no download; code was not locally cached and is omitted (math satisfies the "≥1 additional domain" requirement). Prefill arms: 15,360–16,384 tokens/domain or 16,384 (cliff). No training, no model download, no second checkpoint.

### The three arms (all measured forward passes)

**Q2-A — cross-domain tolerance: `GO`.** The Q1-B additive-depth gate holds on both domains. At L=8 conditional-on-affected: math KL 0.0090 (top-1 97.9%), WikiText-2 KL 0.0145 (top-1 95.0%). Both monotone in L, no super-linear marginal blow-up (ratios 0.90/0.64), zero large divergence at L=8 (186 / 279 affected tokens). No broken domain. The free null-drop tolerance transfers to a reasoning domain.

**Q2-B — decode compounding: divergence observed; NOT degradation (gate numbers alone would flag it, manual text inspection overturns that).** Clean-vs-erased two-stream autoregressive continuation from a shared 256-token prefix over 64 steps, both streams advancing by their own argmax. The numeric gate fails at both L=1 and L=8 (token agreement ≤62.5% vs ≥80% required, mean step KL 3.7/4.5 vs ≤0.05, runaway late/early ratio ~14k–18k× vs ≤3×). But those numbers measure *stream disagreement*, not quality. Replaying the same forward passes and decoding the actual generated text shows the erased stream stays **fluent, on-topic, and meaning-equivalent** — in the replayed case the single divergence is a near-tie flip in a weight conversion (`1.6 → 1.5 lb`) and the rest of the erased continuation is a valid paraphrase of the clean one, not a collapse. The large step-KL after the flip is a trajectory artifact (the streams are generating from different-but-valid contexts), which is why token-level agreement and KL overstate the quality impact.

**Q2-C — cliff mapping: `WITH_MARGIN`.** AX4 nominal cell (incidence 0.009, L=8, 1 expert/layer) is inside the free band: conditional-on-affected mean KL 0.0102, free=True. No incidence crossing within the swept 0.009→0.5 range (KL stays ≤0.0175 through 50% incidence at 1 expert/layer) and no run-length crossing in 8→16; the cliff appears under **multiple experts per layer** — first crossing at 2 experts (KL 0.028, 2× margin over AX4's 1-expert bound). The measured contract stands with margin on its own bounds.

### Interpretation and hand-off (accepted: no non-free regime)

- **Two arms (Q2-A, Q2-C) confirm the measured free tolerance** across an untested domain and up to AX4's boundary: clean.
- **Q2-B is divergence, not degradation.** The decode leg *does* show the streams fly apart after a rare near-tie flip — a reproducibility/traceability fact that matters if the exact generation is what you serve. But it does **not** show quality loss: the actual generated text from the erased stream is a fluent, on-topic paraphrase. The per-step KL and token-agreement gates conflate "the streams no longer agree" with "the output is degraded," and the manual text inspection corrects that. Even the single low-mass drop (L=1), near-free in prefill, can flip a tie and send the stream onto a different-but-valid branch.
- **No calibration is justified.** Because no arm demonstrates a real (non-free) quality degradation, the gated minimal mask-aware calibration sub-step is **not entered**, and Q2 closes as the protocol's scenario (1)/(2) boundary resolved in favor of *no training*. AX4's bounded-run quality contract holds across domains and with margin at its own boundary; generation divergence is a separate concern only if exact output reproduction (vs. meaning) is required.
- **Evidence boundary:** single frozen revision; gsm8k math is the only non-prose domain (code omitted for lack of a local materialization); decode continuation is one prefix at 64 steps; conditional-on-affected and two-stream argmax propagation; quality only — no latency/capacity claim in any arm. Note: the replayed decode diverged at step 5 (weight `1.6→1.5`) while the recorded run's continuation showed first divergence at step 40; the exact flip timing differs across sampling/seed but the qualitative reading (paraphrase, not collapse) is robust. All derived products under `artifacts/runs/q1-quality-erasure/analysis/q2_cross_domain/`, `.../q2_decode/`, and `.../q2_cliff/` (figures under their `figures_q2a|b|c/` subdirs), hashed in `analysis/figure_manifest.json`; run manifest `q2_run_manifest.json`; decoded stream dump at `.../trace/q2_decode_streams.txt` (gitignored, disposable).
- **Researcher decision:** accepted Q2 as measured. No training, no calibration, no guard entered. Q2 closes and the plan advances on the premise that the null-drop tolerance is free across the tested axes. (Post-hoc token inspection was only possible by re-running the decode legs because the original run did not persist decoded sequences — see SOP lesson.)
